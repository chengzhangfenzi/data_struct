...
typedef int data_t;//data_t为int型
typedef struct
{
	data_t data[MAXSIZE];
...
{
    SqStack *stack = (SqStack*)malloc(sizeof(SqStack));//给结构体划分空间，并用指针接收。由于malloc返回的是void空型的指针，而结构体类型的指针接收不到void的指针，所以给结构体划分空间也应该是结构体类型的。
    if(stack==NULL)//判断划分的空间是否分到了，若没有分到，将指针置空，防止野指针的出现。 
    {
        printf("CreateEmptyStack Error\n");
        exit(0);
    }
    stack->top=-1;//置空，使得栈为空没有数据
    return stack;
}
